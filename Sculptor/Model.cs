...
namespace Sculptor
{
    public abstract class Model<T> where T : Model<T>, new()
    {
        ...
        private Dictionary<string, dynamic> PrepareBindings()
        {
            ...
            Dictionary<string, dynamic> bindings = properties.Where(p => p.PropertyType.IsValueType || p.PropertyType == typeof(string)).ToDictionary(p => p.Name.ToSnakeCase(), p => p.GetValue(this));
